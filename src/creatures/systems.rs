...
pub fn animate_death(
    ...
    mut query: Query<(Entity, &Transform, &mut DeathAnimation, &Loot, &Children, Option<&Goblin>), With<Creature>>,
    ball_query: Query<&Transform, With<ResourceBall>>,
) {
    let mut balls_to_magnetize: Vec<(Entity, Vec3)> = Vec::new();

    for (entity, transform, mut death, loot, children, goblin) in &mut query {
        death.timer += time.delta_secs();
        let t = death.timer;

        // Sprite-based creatures (goblins) skip mesh squash effects
        let is_sprite_based = goblin.is_some();
